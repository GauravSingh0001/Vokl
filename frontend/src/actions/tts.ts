...
const S3_BUCKET_URL = env.AWS_S3_BUCKET_NAME
  ? `https://${env.AWS_S3_BUCKET_NAME}.s3.${env.AWS_REGION ?? "ap-south-1"}.amazonaws.com`
  : null;
...
export async function generateSpeech(
  ...
  try {
    ...
    if (!S3_BUCKET_URL) {
      return { success: false, error: "AWS bucket name is not configured" };
    }
